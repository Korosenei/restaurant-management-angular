...
export class EmployeComponent implements OnInit {
  employeObj: USER = new USER();

  listEmployes: USER[] = [];
  filteredEmployes: USER[] = [];
  displayedEmployes: USER[] = [];
  ...
  getEmployes() {
    this.http.get<USER[]>('http://localhost:2028/users/filter/role/CLIENT').subscribe({
      next: (res) => {
  ...
  exportToPDF() {
    ...
    if (confirm('Voulez-vous vraiment exporter les employés en PDF ?')) {
      ...
      const data = this.filteredEmployes.map((employe) => [
        employe.id,
        employe.matricule,
